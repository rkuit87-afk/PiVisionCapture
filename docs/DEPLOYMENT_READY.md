...
cd ~/PiVisionCapture
python gpio_trigger.py --cam-capture "rtsp://root:<CAMERA_PASSWORD>@192.168.3.145/live1s1.sdp"
```
...
- **Left (monitor)**: 192.168.3.146 (credentials in CAM_LEFT_RTSP_URL env var, not committed)
- **Right (capture)**: 192.168.3.145 (credentials in CAM_RIGHT_RTSP_URL env var, not committed)
